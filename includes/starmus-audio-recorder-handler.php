...
if (!class_exists('Starmus_Audio_Submission_Handler')) {
    ...
    class StarmusAudioSubmissionHandler
    {
        ...
        public function handle_submission(): void
        {
            ...
            $signature_id = 0;
            if ( ! empty( $_FILES['contributor_signature'] ) && UPLOAD_ERR_OK === $_FILES['contributor_signature']['error'] ) {
                $signature_id = $this->upload_file_to_media_library( 'contributor_signature' );
            ...
            $meta_input = [
                'audio_id_js'               => $audio_uuid_from_js,
                'audio_consent'             => 'yes',
                '_audio_attachment_id'      => $attachment_id,
                'contributor_ip'            => $current_ip_address,
                'contributor_user_agent'    => $current_user_agent,
                'agreement_datetime'        => $current_submitted_at,
                'submission_id'             => $submission_id,
                'audio_submission_event_id' => $current_audio_submission_id,
                ...
                'audio_file_name_original'  => $file['name'],
                'audio_files_originals'     => [ $attachment_id ],
            ];

            if ( $signature_id ) {
                $meta_input['contributor_signature'] = $signature_id;
            }

            $additional_fields = [
                'agreed_to_contributor_terms',
                'contributor_id',
                'contributor_name',
                'url',
                'contributor_geolocation',
            ...
            foreach ( $additional_fields as $field_key ) {
                if ( isset( $_POST[ $field_key ] ) ) {
                    ...
                    if ( is_array( $value ) ) {
                        $meta_input[ $field_key ] = array_map( 'sanitize_text_field', (array) $value );
                    } else {
                        $meta_input[ $field_key ] = 'url' === $field_key ? esc_url_raw( $value ) : sanitize_text_field( $value );
                    }
                }
            }
            ...
            update_post_meta( $post_id, 'audio_file_name_original', $file['name'] );
            if ( $signature_id ) {
                update_post_meta( $post_id, 'contributor_signature', $signature_id );
            }
