...
(function ($) {
  ...
  audio.addEventListener('loadedmetadata', function() {
    ...
    Peaks.init(peaksOptions, function (err, peaks) {
      ...
      audio.addEventListener('play', () => { btnPlay.textContent = 'Pause'; btnPlay.setAttribute('aria-pressed', 'true'); });
      audio.addEventListener('pause', () => { btnPlay.textContent = 'Play'; btnPlay.setAttribute('aria-pressed', 'false'); });
      ...
      document.addEventListener('keydown', (e)=>{
        ...
        if (e.key==='='){ e.preventDefault(); peaks.zoom.zoomIn(); }
      ...
      function renderRegions(){
        ...
        regs.forEach(seg=>{
          ...
          const tr  = document.createElement('tr');
          tr.innerHTML = `
            <td><input data-k="label" data-id="${seg.id}" value="${seg.label ? String(seg.label).replace(/"/g,'&quot;') : ''}" maxlength="200" placeholder="Annotation" class="widefat" /></td>
