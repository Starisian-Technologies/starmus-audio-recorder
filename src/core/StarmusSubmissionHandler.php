...
final class StarmusSubmissionHandler implements IStarmusSubmissionHandler
{
    ...
    public function handle_upload_chunk_rest_base64(WP_REST_Request $request): array|WP_Error
    {
        try {
            ...
            if (empty($params['file_data']) || empty($params['filename'])) {
                return $this->err('missing_data', 'Missing file data or filename.', 400);
            }

            $file_data = $params['file_data'];
            // Remove data URI scheme if present
            if (str_contains((string) $file_data, ',')) {
                $file_data = explode(',', (string) $file_data)[1];
            }

            $decoded = base64_decode((string) $file_data, true);
            if (false === $decoded) {
                return $this->err('invalid_base64', 'Invalid base64 data.', 400);
            }
            ...
            if (file_put_contents($temp_file, $decoded) === false) {
                return $this->err('write_failed', 'Failed to write temp file.', 500);
            }
            ...
            if (empty($form_data['filetype'])) {
                ...
                $form_data['filetype'] = $check['type'] ?: 'application/octet-stream';
            }

            return $this->process_completed_file($temp_file, $form_data);
        } catch (Throwable $throwable) {
            StarmusLogger::log($throwable, ['component' => self::class, 'method' => __METHOD__]);
            return $this->err('server_error', 'Base64 upload failed.', 500);
        }
    }
    ...
    {
        return ($this->settings instanceof StarmusSettings && $this->settings->get('cpt_slug')) ? sanitize_key((string) $this->settings->get('cpt_slug')) : 'audio-recording';
    }
