...
class AudioProcessingService {
    ...
    private function write_id3_tags( int $attachment_id, string $file_path ): bool {
        ...
        if ( ! $attachment_post ) return false;

        // --- Step 1: Baseline tags ---
        $tag_data = [
            'title'   => [ $attachment_post->post_title ],
            'artist'  => [ get_the_author_meta( 'display_name', $attachment_post->post_author ) ],
            'album'   => [ get_bloginfo( 'name' ) ],
            'year'    => [ get_the_date( 'Y', $attachment_post ) ],
            'comment' => [ "Recorded via Starmus plugin. Attachment ID: {$attachment_id}." ],
        ];
        ...
        if ( $tagwriter->WriteTags() ) return true;
