...
class StarmusAudioProcessingService
{
    ...
    public function process_attachment(int $attachment_id, array $args = []): array
    {
        ...
        try {
            $source_path = (new $this->files())->get_local_copy($attachment_id);
            if (! $source_path || ! file_exists($source_path)) {
                $this->dal->set_audio_state($attachment_id, 'error_source_missing');
                StarmusLogger::error('StarmusAudioProcessingService', 'Missing source file', ['attachment_id' => $attachment_id, 'source_path' => $source_path]);
                return $result;
            }
            ...
            if (! $wav_path || ! file_exists($wav_path)) {
                ...
                StarmusLogger::warning('StarmusAudioProcessingService', 'WAV conversion failed (continuing)', ['attachment_id' => $attachment_id, 'wav_path' => $wav_path]);
            }
            ...
            try {
                ...
                if (method_exists($this->dal, 'persist_audio_outputs')) {
                    ...
                    StarmusLogger::info('StarmusAudioProcessingService', 'Persisted audio outputs via DAL', ['attachment_id' => $attachment_id, 'mp3_path' => $mp3_path, 'wav_path' => $wav_path]);
                } else {
                    // Fallback: attempt to use save_post_meta on attachment id directly
                    StarmusLogger::warning('StarmusAudioProcessingService', 'DAL missing persist_audio_outputs method; falling back to direct meta updates', ['attachment_id' => $attachment_id]);
            ...
            try {
                ...
                if ($recording_id > 0) {
                    ...
                    if (method_exists($this->dal, 'save_audio_outputs')) {
                        ...
                        StarmusLogger::warning('StarmusAudioProcessingService', 'DAL missing save_audio_outputs; falling back to update_field/update_post_meta', ['recording_id' => $recording_id]);
                    ...
                    StarmusLogger::warning('StarmusAudioProcessingService', 'No parent recording found for attachment; cannot save outputs to recording post', ['attachment_id' => $attachment_id]);
                }
            } catch (\Throwable $e) {
                StarmusLogger::error(
                    'StarmusAudioProcessingService',
    ...
    private function write_id3_tags(int $attachment_id, string $file_path): bool
    {
        $post = get_post($attachment_id);
        if (! $post) {
            StarmusLogger::warning('StarmusAudioProcessingService', 'Attachment not found for ID3', ['attachment_id' => $attachment_id]);
            return false;
        }
        ...
        $tags = [
            ...
            'album'   => [get_bloginfo('name')],
            'year'    => [get_the_date('Y', $post)],
            'comment' => [\sprintf('Recorded via Starmus (Attachment %d)', $attachment_id)],
        ];
