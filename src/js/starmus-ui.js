...
function render(state, elements) {
    ...
    if (status === "calibrating" || status === "recording") {
        const vol =
            status === "calibrating" ? calibration.volumePercent || 0 : recorder.amplitude || 0;
...
function initInstance(store, incomingElements, forcedInstanceId) {
    ...
    safeBind(el.continueBtn, "click", function () {
        ...
        for (let i = 0; i < inputs.length; i++) {
            ...
            // Checkbox must be checked; text/select must have value
        ...
        if (valid) {
            ...
            // Show verbal error message as requested
            if (el.messageBox) {
