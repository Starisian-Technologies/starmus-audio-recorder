...
function getSharedContext() {
  ...
  if (!sharedAudioContext || sharedAudioContext.state === 'closed') {
    ...
    // EXPOSE FOR INTEGRATOR WATCHDOG
...
// --- FIX: Force Wake Helper ---
async function starmusEnsureContext() {
    const ctx = getSharedContext();
    if (ctx.state === 'suspended') {
        console.log('[Recorder] Forcing AudioContext resume...');
        await ctx.resume();
    }
    return ctx;
}
...
async function calibrateAudioLevels(stream, onUpdate) {
  ...
  const samples = [];
  const DURATION = 6000; // REDUCED FROM 15000 FOR BETTER UX
...
export function initRecorder(store, instanceId) {
  ...
  if(CommandBus.unsubscribeInstance) CommandBus.unsubscribeInstance(instanceId);
  ...
  CommandBus.subscribe('setup-mic', async (_payload, meta) => {
    ...
    try {
      ...
      // FIX: Force wake-up here
      await starmusEnsureContext();
  ...
  CommandBus.subscribe('start-recording', async (_payload, meta) => {
    ...
    // Soft check for calibration to avoid blocking recording
    if (state.status === 'calibrating') {
      console.warn('[Recorder] Calibrating... wait');
    }

    try {
      const rawStream = await navigator.mediaDevices.getUserMedia({ audio: true });
      
      // FIX: Force wake-up here again
      await starmusEnsureContext();
