...
(function (window, document) {
  ...
  function debugInitBanner() {
    ...
    banner.style.cssText =
      "position:fixed;top:0;left:0;z-index:99999;background:#222;color:#fff;padding:4px 12px;font:14px monospace;opacity:0.95";
  ...
  function startTimer(instanceId) {
    if (!safeId(instanceId) || timers[instanceId]) {
      return;
    }
    ...
    timers[instanceId] = setInterval(() => {
      ...
      if (
        ...
        console.log("[Starmus] Timer tick", {
          instanceId,
  ...
  function buildRecorderUI(instanceId) {
    ...
    const submitBtn = document.querySelector(
      `#${instanceId} #starmus_submit_btn_${instanceId}`,
    );
    ...
    const handleCalibration = () => {
      ...
      window.StarmusAudioRecorder.calibrate(
        instanceId,
        (message, volume, isDone) => {
          if (message) {
            statusArea.textContent = message;
          }
          const volumeEl = el(`starmus_volume_level_${instanceId}`);
          if (volumeEl && typeof volume === "number") {
    ...
    recordBtn.addEventListener("click", () => {
      ...
      const typeSelect = document.querySelector(
        `#${instanceId} [name="audio_file_type"]`,
      );
      if (typeSelect && typeSelect.value) {
        selectedType = typeSelect.value;
      }
      window.StarmusAudioRecorder.startRecording(instanceId, selectedType);
      window.StarmusAudioRecorder.startVolumeMonitoring(
        instanceId,
        (volume) => {
          const volumeEl = el(`starmus_volume_level_${instanceId}`);
          if (volumeEl) {
    ...
    stopBtn.addEventListener("click", () => {
      ...
      window.StarmusAudioRecorder.stopRecording(instanceId)
        .then((audioBlob) => {
          log("info", "Stop recording resolved. Blob ready.", {
            size: audioBlob?.size,
          });
  ...
  function handleContinueClick(formId) {
    ...
    if (!window.StarmusSubmissionsHandler?.initRecorder) {
      showUserMessage(
        formId,
        "A critical component is missing. Please reload.",
        "error",
      );
    ...
    window.StarmusSubmissionsHandler.initRecorder(formId)
      ...
      .catch((err) => {
        showUserMessage(
          formId,
          "Could not initialize microphone. Please check permissions and try again.",
          "error",
        );
        step1.style.display = "block";
        step2.style.display = "none";
  ...
  function initializeForm(form) {
    ...
    form.addEventListener("submit", (event) => {
      ...
      if (!window.StarmusSubmissionsHandler?.handleSubmit) {
        showUserMessage(
          formId,
          "Cannot submit. A critical component is missing.",
          "error",
        );
        return;
      }
      ...
      window.StarmusSubmissionsHandler.handleSubmit(formId, form).finally(
        () => {
          isSubmitting = false;
        },
      );
  ...
  function init() {
    if (window.starmusUiInitialized) {
      return;
    }
