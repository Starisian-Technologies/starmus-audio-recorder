    ...
    async function handleSubmit(formFields) {
        ...
        try {
            ...
            if (useTus) {
                ...
                const uploadUrl = await uploadWithTus(audioBlob, fileName, formFields, metadata, instanceId, onProgress);
                store.dispatch({ type: 'starmus/submit-complete', payload: { uploadUrl } });
            } else {
                debugLog('[Upload] Using Direct...');
                const response = await uploadDirect(audioBlob, fileName, formFields, metadata, instanceId, onProgress);
