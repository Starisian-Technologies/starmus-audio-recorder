...
class SmartAudioPlayer {
  ...
  constructor(config = {}) {
    ...
    this.canPlayOpus = this.audioElement
   ...
   */
  log(...args) {
    if (this.config.debug) console.log('[SmartAudio]', ...args);
  }
  ...
  evaluateDeviceTier() {
    // 1. Check Hardware
    if (navigator.deviceMemory && navigator.deviceMemory < this.config.lowMemoryLimit) return true;
    if (navigator.hardwareConcurrency && navigator.hardwareConcurrency < this.config.lowCoreLimit) return true;
