  ...
  async function checkAndSync() {
    ...
    try {
      ...
      allItemsReq.onsuccess = async () => {
        ...
        log(
          "info",
          `Found ${pendingItems.length} pending submission(s). Starting upload process.`,
        );
        ...
        for (const item of pendingItems) {
          try {
            ...
            if (
              ...
              typeof window.StarmusSubmissionsHandler.resumableTusUpload ===
                "function"
            ) {
              ...
              log(
                "info",
                `Successfully synced and removed item ${item.id} from queue.`,
              );
            } else {
              log(
                "warn",
                "Upload handler not found. Cannot sync item.",
                item.id,
              );
            }
          } catch (uploadError) {
            log(
              "error",
              `Failed to sync item ${item.id}. It will be retried later.`,
              uploadError,
            );
