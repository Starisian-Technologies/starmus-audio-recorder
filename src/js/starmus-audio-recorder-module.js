...
(function (window) {
  ...
  function debugInitBanner() {
    ...
    banner.style.cssText =
      "position:fixed;top:48px;left:0;z-index:99999;background:#22a;color:#fff;padding:4px 12px;font:14px monospace;opacity:0.95";
  ...
  const hasMediaRecorder = !!(
    window.MediaRecorder && window.navigator.mediaDevices
  );
  ...
  function secureLog(level, msg, data) {
    if (console && console[level]) {
      console[level]("[Starmus Recorder]", msg, data || "");
    }
  }
  ...
  window.StarmusAudioRecorder = {
    ...
    calibrate: function (instanceId, onUpdateCallback) {
      ...
      function tick() {
        ...
        if (elapsed < 5000) {
          onUpdateCallback(
            "Be quiet for background noise (" + remaining + "s)",
          );
        } else if (elapsed < 10000) {
          onUpdateCallback("Now speak clearly and loudly (" + remaining + "s)");
        } else {
          onUpdateCallback("Be quiet again (" + remaining + "s)");
        }
      ...
      function done() {
        ...
        const quietSamples = samples.slice(
          0,
          Math.floor(samples.length * 0.33),
        );
        const speechSamples = samples.slice(
          Math.floor(samples.length * 0.33),
          Math.floor(samples.length * 0.67),
        );

        const noiseFloor =
          quietSamples.reduce((a, b) => a + b, 0) / quietSamples.length;
        const speechLevel =
          speechSamples.reduce((a, b) => a + b, 0) / speechSamples.length;
        ...
        gain = applyFilters("starmus_calibration_gain", gain, instanceId, {
          speechLevel,
          noiseFloor,
          snr,
        });
        ...
        instance.gain.gain.setTargetAtTime(
          gain,
          instance.ctx.currentTime,
          0.01,
        );
        instance.isCalibrated = true;

        onUpdateCallback(
          "Ready to record. Mic calibrated (gain ×" +
            gain.toFixed(1) +
            ", SNR: " +
            snr.toFixed(1) +
            ").",
    ...
    startVolumeMonitoring: function (instanceId, onVolumeChange) {
      ...
      function update() {
        ...
        instance.avgVolume =
          instance.volumeSamples.reduce((a, b) => a + b, 0) /
          instance.volumeSamples.length;
        ...
        onVolumeChange(
          applyFilters("starmus_volume_level", volume, instanceId),
        );
    ...
    startRecording: function (instanceId, selectedTypeOrLang) {
      ...
      if (
        ...
        instances[instanceId].isRecording
      ) {
        return;
      }
      const instance = instances[instanceId];
      console.log("[Starmus] startRecording called", {
        instanceId,
      ...
      try {
        ...
        console.log("[Starmus] startRecording set startTime", {
          instanceId,
          startTime: instance.startTime,
        ...
        instance.recorder.onstop = () => {
          ...
          if (
            instance.recorder &&
            instance.recorder.mimeType &&
            instance.recorder.mimeType.startsWith("audio/")
          ) {
    ...
    stopRecording: function (instanceId) {
      if (!isSafeId(instanceId) || !(instanceId in instances)) {
        return Promise.reject(
          new Error("Invalid instance ID for stopRecording."),
        );
      }
      const instance = instances[instanceId];
      ...
      return new Promise((resolve) => {
        instance.recorder.onstop = () => {
          ...
          secureLog("info", "Blob created", {
            size: instance.audioBlob.size,
            type: instance.audioBlob.type,
          });
        ...
        if (
          ...
          instance.speechRecognition.readyState !== "ended"
        ) {
    ...
    getRecordingQuality: function (instanceId) {
      ...
      return {
        ...
        quality:
          instance.peakVolume > 20 &&
          instance.avgVolume > 10 &&
          silenceRatio < 0.7
            ? "good"
            : "poor",
        warnings: [
    ...
    getSubmissionData: function (instanceId) {
      ...
      if (instance.audioBlob) {
        ...
        try {
          ...
          const form =
            document.getElementById("starmus_recorder_form_" + instanceId) ||
            document.querySelector('form[id^="starmus_recorder_form"]');
          if (form) {
            const titleInput = form.querySelector(
              'input[name="starmus_title"]',
            );
        ...
        const data = {
          ...
          metadata: {
            ...
            quality: {
              ...
              volumeConsistency:
                instance.volumeSamples.length > 0
                  ? instance.volumeSamples.reduce(
                      (acc, v) => acc + Math.abs(v - instance.avgVolume),
                      0,
                    ) / instance.volumeSamples.length
                  : 0,
            },
            ...
            transcript: instance.transcript.map((t) => ({
              ...
              timestampUTC: new Date(
                instance.startTime + t.timestamp,
              ).toISOString(),
    ...
    validateWestAfricanLanguage: function (
      instanceId,
      recordingType = "unknown",
    ) {
      if (!isSafeId(instanceId) || !(instanceId in instances)) {
        return { isValid: false, reason: "Invalid instance" };
      }
      const instance = instances[instanceId];
      const duration = Date.now() - instance.startTime;

      const allText = instance.transcript
        .map((t) => t.text)
        .join(" ")
        .toLowerCase()
        .trim();
      ...
      const avgConfidence =
        instance.transcript.length > 0
          ? instance.transcript.reduce((sum, t) => sum + t.confidence, 0) /
            instance.transcript.length
          : 0;
      ...
      if (
        typeContext.includes("word") ||
        (duration < 3000 && totalWords <= 2)
      ) {
        const obviousEnglish =
          /^(hello|yes|no|the|and|is|are|you|me|my|your|good|bad|big|small|one|two|three)$/i.test(
            allText,
          );
        const obviousFrench =
          /^(bonjour|oui|non|le|la|les|et|est|vous|moi|mon|votre|bon|mauvais|grand|petit|un|deux|trois)$/i.test(
            allText,
          );
        ...
        if (avgConfidence > 0.95 && (obviousEnglish || obviousFrench)) {
          return {
            isValid: false,
            reason: `Obvious ${obviousEnglish ? "English" : "French"} word detected`,
            recordingType: "word",
          };
        }
        return {
          isValid: true,
          reason: "Single word - likely West African",
          recordingType: "word",
          avgConfidence,
        };
      }
      ...
      if (
        typeContext.includes("phrase") ||
        typeContext.includes("proverb") ||
        typeContext.includes("saying") ||
        (duration < 15000 && totalWords <= 15)
      ) {
        const englishPattern =
          /\b(the|and|is|in|to|of|a|that|it|with|for|as|was|on|are|you|this|have|they|we|said|do|will|about|then|would|make|what|know|also|your|can|now|when|where|good|come|get|see|way|who|say)\b/g;
        const frenchPattern =
          /\b(le|la|les|de|des|du|et|est|dans|pour|avec|sur|sont|vous|que|qui|une|un|ce|ne|pas|tout|être|avoir|faire|dire|aller|voir|savoir|venir|vouloir|donner|parler)\b/g;
        ...
        const recognizedRatio =
          Math.max(englishMatches, frenchMatches) / Math.max(totalWords, 1);

        if (recognizedRatio > 0.6 && avgConfidence > 0.8) {
          return {
            isValid: false,
            reason: `${Math.round(recognizedRatio * 100)}% ${englishMatches > frenchMatches ? "English" : "French"} phrase patterns`,
            recordingType: "phrase",
          };
        }
        return {
          isValid: true,
          reason: "Phrase/proverb - likely West African",
          recordingType: "phrase",
          avgConfidence,
        };
      }
      ...
      const englishPattern =
        /\b(the|and|is|in|to|of|a|that|it|with|for|as|was|on|are|you|this|have|from|they|we|been|their|said|which|do|how|will|about|many|then|some|would|make|like|has|more|what|know|first|also|your|work|only|can|should|now|here|when|where|good|come|could|get|new|see|way|who|did|say|too|any)\b/g;
      const frenchPattern =
        /\b(le|la|les|de|des|du|et|est|dans|pour|avec|sur|sont|vous|que|qui|une|un|ce|se|ne|pas|tout|être|avoir|faire|dire|aller|voir|savoir|prendre|venir|vouloir|pouvoir|croire|donner|parler|porter|finir|partir|sentir|sortir|conduire)\b/g;
      ...
      if (recognizedRatio > 0.4 && avgConfidence > 0.7) {
        return {
          isValid: false,
          reason: `${Math.round(recognizedRatio * 100)}% ${englishMatches > frenchMatches ? "English" : "French"} patterns detected`,
          recordingType: "story",
        };
      }
      ...
      return {
        isValid: true,
        reason: "Recording validated - likely West African language",
        recordingType: "unknown",
        avgConfidence,
      };
    },
