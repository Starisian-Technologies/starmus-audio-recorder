...
final class StarmusSubmissionHandler
{
	...
	{
		try {
			StarmusLogger::debug('SubmissionHandler', 'Saving all metadata', array('post_id' => $audio_post_id));
			...
			$env_snapshot = $metadata['environmentSnapshot'] ?? array();
			...
			if ($is_offline_sync && in_array($network_type, array('2g', 'slow-2g'), true)) {
