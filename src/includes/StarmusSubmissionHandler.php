...
final class StarmusSubmissionHandler
{
    ...
    public function save_all_metadata(int $audio_post_id, int $attachment_id, array $form_data): void
    {
        try {
            ...
            StarmusLogger::error('SubmissionHandler', 'Metadata Save Failed', ['error' => $throwable->getMessage()]);
        }
    }
    ...
    private function trigger_post_processing(int $post_id, int $attachment_id, array $params): void
    {
        try {
            ...
            if (!$processor->process($post_id, $attachment_id, $params) && ! wp_next_scheduled('starmus_cron_process_pending_audio', [$post_id, $attachment_id])) {
