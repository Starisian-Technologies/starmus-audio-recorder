...
class StarmusAudioEditorUI {
	...
	public function enqueue_scripts(): void {
		...
		if ( ! $has_shortcode && ! str_contains( $content, '[starmus_audio_editor' ) ) {
			return;
		}
		...
		wp_enqueue_style( 'starmus-audio-editor-style', STARMUS_URL . 'assets/css/starmus-audio-editor-style.min.css', array(), STARMUS_VERSION );
		wp_enqueue_script( 'peaks-js', STARMUS_URL . 'assets/js/vendor/peaks.min.js', array(), '2.0.0', true );
		wp_enqueue_script( 'starmus-audio-editor', STARMUS_URL . 'assets/js/starmus-audio-editor.min.js', array( 'peaks-js', 'jquery' ), STARMUS_VERSION, true );
	...
	public function handle_save_annotations( \WP_REST_Request $request ): \WP_REST_Response {
		...
		if ( get_transient( $key ) ) {
			return new \WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Please wait before saving again.', 'starmus_audio_recorder' ),
				),
				429
			);
		}
		...
		foreach ( (array) $annotations as $seg ) {
			if ( empty( $seg['id'] ) || ! isset( $seg['startTime'], $seg['endTime'] ) ) {
				continue;
			}
